...
- **心跳发送策略**：每2秒发送一次心跳消息，使用独立的 Ping 消息类型
...
- **帧头结构**：固定5字节头部，包含帧类型和数据长度
- **帧体结构**：直接传输原始二进制数据，无需额外序列化
- **性能优势**：
  - 头部仅5字节（相比原方案9字节HeadMsg + bincode序列化的TextQuicMsg开销）
  - 避免了bincode序列化带来的额外开销
...
- **专用接收循环**：使用轻量级MediaFrameHeader协议，替代通用的TextQuicMsg反序列化
...
- **帧构建**：使用MediaFrameHeader::build_frame直接构建完整帧
- **重试逻辑**：首次通话时等待MediaData通道就绪，最多重试6次
...
- **连接建立**：创建Endpoint、配置客户端、连接服务器、开启双向流
...
- **结构定义**：包含远程地址、QUIC发送流、是否为服务端、通道类型
- **流注册**：每个通道类型对应一个独立的发送流
- **流访问**：通过目标用户UUID和通道类型获取对应的发送流
- **流清理**：连接结束时关闭并清理所有发送流
