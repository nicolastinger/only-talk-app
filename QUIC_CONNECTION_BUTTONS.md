...
为前端应用添加了两个新的控制按钮，用于管理 QUIC 连接状态：

1. **断开连接按钮** - 清理和服务器连接的 QUIC 资源
2. **重新连接按钮** - 重新建立和服务器连接的 QUIC 资源
...
#### 1. 断开 QUIC 连接（DisconnectOutlined 图标）

- **功能**: 断开当前所有 QUIC 连接，清理相关资源
- **操作**: 点击后会：
  - 清理 QUIC 服务器连接列表（GLOBAL_QUIC_SERVER_LIST）
  - 标记用户离线状态（quic_disconnected = true）
  - 停止心跳发送
- **反馈**: 操作成功后显示"QUIC 连接已断开"提示

#### 2. 重新连接 QUIC（ReloadOutlined 图标）

- **功能**: 重新建立与服务器的 QUIC 连接
- **操作**: 点击后会：
  - 先断开现有连接（调用 disconnect_quic）
  - 清除断开状态标记（quic_disconnected = false）
  - 重新启动 QUIC 客户端连接
- **反馈**: 操作成功后显示"QUIC 重连请求已发送"提示
...
按钮被添加到工具栏的悬停菜单中，与其他工具按钮（主题切换、语言切换等）保持一致的 UI 风格。
...
3. **状态管理**:
   ...
   - 可以通过 Tauri 命令 `get_user_map` 查询当前状态
