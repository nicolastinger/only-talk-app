...
| `level2` | 子功能模块 | 1=好友模块，2=用户通知，3=群组模块，4=交友广场模块，5=动态广场模块 |
...
#### level1=1, level2=4 — 交友广场模块

| level3 | level4 | 名称 | 含义 | 后端构造函数 | 客户端处理 |
|--------|--------|------|------|-------------|-----------|
| 1 | 0 | 心动通知 | 有人对你心动了 | `send_plaza_like_msg()` | 实时推送，前端增加广场未读角标 |
| 2 | 0 | 互相心动(匹配)通知 | 双方互相心动，可开始交友 | `send_plaza_match_msg()` | 实时推送，前端增加广场未读角标 |

#### level1=1, level2=5 — 动态广场模块

| level3 | level4 | 名称 | 含义 | 后端构造函数 | 客户端处理 |
|--------|--------|------|------|-------------|-----------|
| 1 | 0 | 动态点赞通知 | 有人赞了你的动态 | `send_moment_like_msg()` | 实时推送，前端增加动态未读角标 |
| 2 | 0 | 动态评论通知 | 有人评论了你的动态 | `send_moment_comment_msg()` | 实时推送，前端增加动态未读角标 |
...
前端 useSystemNotify.ts
    ...
    ├── level1=1, level2=4 ──▶ 广场未读角标
    ├── level1=1, level2=5 ──▶ 动态未读角标
