...
fn main() {
    ...
    if let Some((time, speed, mem)) = zune_result {
        println!("{:<20} {:<15} {:<15} {:<15}", "zune-jpeg", format!("{:?}", time), format!("{:.2} MP/s", speed), format!("{:.2} MB", mem));
    }
    
    if let Some((time, speed, mem)) = image_rs_result {
        println!("{:<20} {:<15} {:<15} {:<15}", "image-rs", format!("{:?}", time), format!("{:.2} MP/s", speed), format!("{:.2} MB", mem));
    }
