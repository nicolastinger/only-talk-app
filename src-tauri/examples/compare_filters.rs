use image::{DynamicImage, ImageBuffer, Rgb, Rgba, Luma, imageops::FilterType};
use std::fs::File;
use std::io::Read;
use std::path::Path;
...
fn main() {
    ...
    for (name, filter) in filters {
        ...
        if let Some((time, output_size, quality)) = result {
            println!("{:<15} {:<15} {:<15} {:<15}", name, format!("{:?}", time), format!("{:.2} KB", output_size as f64 / 1024.0), quality);
        }
    }
...
fn benchmark_resize(img: &DynamicImage, target_size: u32, filter: FilterType, name: &str) -> Option<(std::time::Duration, usize, &'static str)> {
...
fn decode_jpeg(input_path: &Path) -> Result<DynamicImage> {
    ...
    match components {
        ...
        _ => {
            Err(anyhow::anyhow!("Unsupported JPEG components: {}", components))
        }
    }
}
