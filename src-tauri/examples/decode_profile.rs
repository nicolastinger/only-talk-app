...
fn analyze_decode_steps(input_path: &Path) {
    ...
    println!(
        "  耗时: {:?} ({:.2} MB/s)",
        read_time,
        buffer.len() as f64 / 1024.0 / 1024.0 / read_time.as_secs_f64()
    );
    ...
    println!(
        "  解码速度: {:.2} MP/s",
        (info.width as u64 * info.height as u64) as f64 / 1_000_000.0 / decode_time.as_secs_f64()
    );
    ...
    println!(
        "  - 创建解码器: {:?} ({:.1}%)",
        create_time,
        create_time.as_secs_f64() / total_time.as_secs_f64() * 100.0
    );
    println!(
        "  - 获取信息: {:?} ({:.1}%)",
        info_time,
        info_time.as_secs_f64() / total_time.as_secs_f64() * 100.0
    );
    println!(
        "  - 解码数据: {:?} ({:.1}%)",
        decode_time,
