use image::{DynamicImage, ImageBuffer, Rgba, Rgb, Luma, ImageReader, imageops::FilterType};
use std::fs::File;
...
fn compress_with_detailed_timing(input_path: &Path) -> Result<()> {
    ...
    println!("  输出大小: {} bytes ({:.2} KB)\n", final_data.len(), final_data.len() as f64 / 1024.0);
    ...
    println!("  - 解码: {:?} ({:.1}%)", decode_time, decode_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("  - 缩放: {:?} ({:.1}%)", total_time - decode_time - encode_time - write_time, (total_time - decode_time - encode_time - write_time).as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("  - 编码: {:?} ({:.1}%)", encode_time, encode_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("  - 写入: {:?} ({:.1}%)", write_time, write_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
