...
use crate::service::chat_service::{
    create_chat_session_service, get_chat_session_from_db, send_msg, update_last_read_msg_from_db,
};
...
use crate::store::chat_record_db::{
    ...
    query_friend_info, query_friend_info_by_id,
};
...
pub async fn get_friend_info(friend_uuid: String) -> Result<FriendVo, String> {
    let me = get_user_info(&"uuid".to_string())
        .await
        .map_err(|e| e.to_string())?;
    let friend = query_friend_info_by_id(&me, &friend_uuid)
        .await
        .map_err(|e| e.to_string())?;
