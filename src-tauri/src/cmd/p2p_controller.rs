...
use crate::service::p2p_service::{
    ...
    send_p2p_text_msg_service, send_p2p_video_call_invite_service, send_p2p_video_config_service, send_p2p_video_frame_service,
};
...
/// 用于发起视频通话时通知对端
#[tauri::command]
pub async fn send_p2p_video_call_invite(target_uuid: String) -> Result<(), String> {
    send_p2p_video_call_invite_service(target_uuid, None)
        .await
        .map_err(|e| e.to_string())
}
