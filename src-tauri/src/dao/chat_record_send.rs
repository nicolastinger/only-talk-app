...
/// 插入一条发送记录
pub async fn insert_chat_record_send(
    chat_record_send: &ChatRecordSend,
) -> Result<(), anyhow::Error> {
...
// 标记消息已发送完成
pub async fn update_chat_record_send_success(
    send_id: &str,
    msg_id: &str,
) -> Result<(), anyhow::Error> {
    let pool_sqlite = get_private_db_client().await?;
...
pub async fn query_chat_record_send_by_user(
    ...
    let mut query_builder = sqlx::query_as::<_, ChatRecordSend>(&query).bind(uuid).bind(recv_user);
