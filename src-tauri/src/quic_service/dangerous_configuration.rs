...
pub fn configure_server() -> ServerConfig {
    ...
    // 创建自签名证书
    let cert = rcgen::generate_simple_self_signed(vec!["localhost".into()]).expect("generate_simple_self_signed failed");
    let cert_der = cert.serialize_der().expect("serialize_der failed");
    let key_der = cert.serialize_private_key_der();

    // 使用自签名证书
    let crypto = crypto.with_single_cert(vec![Certificate(cert_der)], PrivateKey(key_der)).expect("with_single_cert failed");
    ...
    transport_config.max_idle_timeout(Some(Duration::from_secs(180).try_into().expect("Duration::from_secs(180).try_into() failed")));
