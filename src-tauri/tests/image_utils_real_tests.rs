    ...
    match result {
        Ok(_) => {
            ...
            assert!(
                ...
                "Output size should be <= 200KB, got {} bytes",
                output_size
            );

            let mut file_handle = fs::File::open(&output_path).expect("打开输出文件失败");
            let mut header = [0u8; 4];
            file_handle.read_exact(&mut header).expect("读取文件头失败");
